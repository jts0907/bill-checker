...
⚠️ **한계 고지**: 시스템의 제약이나 법령정보 검색이 불가능해 타법령 정보 연계검토가 불가능한 경우에는 다음과 같은 한계 고지를 해야 함. " 법안 본문에 명시된 정보만으로 검토 가능하며, 전체 데이터베이스 접근이 불가하므로 확인 불가 사항은 명시합니다."
